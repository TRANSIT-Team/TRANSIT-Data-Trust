...
module.exports.issue = function issue(payload) {
    ...
    return jwt.sign(
        ...
        process.env.TOKEN_SECRET || fs.readFileSync('/run/secrets/TRANSIT_KONGA_TOKEN_SECRET', 'utf8').trim() // Secret string which will be used to sign the token
    );
};
...
module.exports.verify = function verify(token, next) {
    ...
    return jwt.verify(
        ...
        process.env.TOKEN_SECRET || fs.readFileSync('/run/secrets/TRANSIT_KONGA_TOKEN_SECRET', 'utf8').trim(), // The secret we used to sign it.
