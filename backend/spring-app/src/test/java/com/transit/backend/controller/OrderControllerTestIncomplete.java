...
import com.transit.backend.datalayers.controller.assembler.OrderAssembler;
...
public class OrderControllerTestIncomplete extends BaseIntegrationTest {
	...
	//@InjectMocks
	//OrderOrderClassController orderClassController;
	@Autowired
	...
	public OrderControllerTestIncomplete(@Autowired KeycloakServiceManager keycloakServiceManager) {
	...
	@Test
	public void generateOrderWithoutPackages() throws IOException {
		
		setupOrderWithoutPackage(getBASE_PATH() + getRandomServerPort() + "/api/v1");
		ParameterizedTypeReference<PagedModel<OrderDTO>> responseType = new ParameterizedTypeReference<>() {
		};
		ResponseEntity<String> res = getRestTemplateGenerator().getTestRestTemplate().getForEntity(path + "/orders/", String.class);
		log.info("rebody" + res.getBody());
		res = getRestTemplateGenerator().getTestRestTemplate().getForEntity(path + "/orders/", String.class);
		log.info("rebody" + res.getBody());
		
		
		ResponseEntity<PagedModel<OrderDTO>> result = configurer.registerHypermediaTypes(getRestTemplateGenerator().getTestRestTemplate().getRestTemplate()).exchange(path + "/orders/", HttpMethod.GET, HttpEntity.EMPTY, responseType);
		//var x = result.getBody();
		//ObjectMapper objectMapper = new ObjectMapper();
		//var orders = objectMapper.readValue(objectMapper.readTree(res.getBody()).traverse(), new TypeReference<PagedModel<OrderDTO>>() {});
		Collection<OrderDTO> searchResult = Objects.requireNonNull(result.getBody()).getContent();
		Assertions.assertEquals(2, searchResult.size());
		for (var order : searchResult) {
			log.info(String.valueOf(order.getPackageItemIds().size()));
		}
		
		
		var order = setupOrderWithoutPackage(getBASE_PATH() + getRandomServerPort() + "/api/v1");
		ResponseEntity<String> res3 = getRestTemplateGenerator().getTestRestTemplate().getForEntity(path + "/orders/", String.class);
		log.info("rebody" + res3.getBody());
		ResponseEntity<String> res2 = getRestTemplateGenerator().getTestRestTemplate().getForEntity(path + "/orders/" + order.getId(), String.class);
		log.info("rebody2" + res2.getBody());
		
	}
	
	Order setupOrderWithoutPackage(String path) {
		this.geoFactory = new GeometryFactory();
	...
	void verifyJsonOrderNotNullValuesOrder(String path, final ResponseEntity<String> response, ResponseEntity<OrderDTO> response2) throws Exception {
		...
		var orderDTO = orderAssembler.toModel(this.orderTest);
		...
		Assertions.assertEquals(Link.of(path + orderDTO.getLink("self").get().getHref()), Link.of(self.get("href").asText()));
	...
	void verifyJsonOrderPropertiesNotNullValues(String path, JsonNode node, OrderDTO orderDTO) throws Exception {
		...
		for (int z = 0; z < this.orderTest.getOrderProperties().size(); z++) {
			...
			Assertions.assertEquals(Link.of(path + orderPropertyDTO.getLink("self").get().getHref()), Link.of(self.get("href").asText()));
	...
	public void putOrder() throws JsonProcessingException {
		...
		var response4 = getRestTemplateGenerator().getTestRestTemplate().exchange(orderPath + this.orderTest.getId() + "/orderstatus", HttpMethod.POST, entityNew, OrderStatusDTO.class);
	...
	public void patchOrder() throws IOException {
		...
		var orderPath = path + "/orders/";
		ResponseEntity<OrderDTO> response3 = getRestTemplateGenerator().getTestRestTemplate().getForEntity(path + "/orders/{id}", OrderDTO.class, this.orderTest.getId());
		Assertions.assertEquals(HttpStatus.OK, response3.getStatusCode());
		var dto = response3.getBody();
	...
	public void deleteOrder() {
		...
		ResponseEntity<Void> response = getRestTemplateGenerator().getTestRestTemplate().exchange(path + "/orders/" + this.orderTest.getId(), HttpMethod.DELETE, null, Void.class);
