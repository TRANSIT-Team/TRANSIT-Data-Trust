...
@Component
public abstract class CrudControllerSubExtraAbstract<test, testDTO extends AbstractDTO<testDTO>, testRoot, testRootDTO, rootController extends CrudControllerExtend<testRootDTO, UUID>, nestedController extends CrudControllerSubResource<testDTO, UUID, UUID>> {
	...
	public abstract AssemblerWrapperSubExtraAbstract<test, testDTO, testRoot, testRootDTO, rootController, nestedController> getAssemblerWrapper();
	
	public ResponseEntity<testDTO> update(
			@PathVariable UUID rootId, UUID id, testDTO dto) throws ClassNotFoundException {
		pingService.available();
		test entity = getMapper().toEntity(dto);
		Optional<AccessResponseDTO> rightsEntry = this.rightsService.getAccess((UUID) id);
	...
	public ResponseEntity<CollectionModel<testDTO>> read(UUID rootId, String query, FilterExtra collectionFilterExtra) {
		...
		if (getFilter()) {
			var newQuery = getFilterExpression.overwriteQueryWithEntityId(query, collectionFilterExtra.isCreatedByMyCompany());
			if (newQuery == null) {
